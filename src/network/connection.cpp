...
Connection::Connection(asio::ip::tcp::socket socket, tinykv::storage::Storage* storage)
    ...
    asio::async_read(socket_, asio::buffer(&network_length_, 4),
        [this, self](std::error_code ec, std::size_t /*length*/) {
            if (!ec) {
                ...
                if (body_length > 10 * 1024 * 1024) { // 10MB limit check
    ...
    asio::async_read(socket_, asio::buffer(buffer_),
        [this, self](std::error_code ec, std::size_t /*length*/) {
            if (!ec) {
                ...
                if (tinykv::protocol::Message::Decode(buffer_, req)) {
                    ProcessRequest(req);
                } else {
...
void Connection::ProcessRequest(const tinykv::protocol::Message& req) {
    tinykv::protocol::Message resp;
    resp.type = tinykv::protocol::MessageType::kResponse;

    switch (req.type) {
        case tinykv::protocol::MessageType::kGet: {
        ...
        case tinykv::protocol::MessageType::kPut: {
            bool success = storage_->Put(req.key, req.value);
            resp.status = success;
            break;
        }
        case tinykv::protocol::MessageType::kDelete: {
...
void Connection::SendResponse(const tinykv::protocol::Message& resp) {
    auto self(shared_from_this());
