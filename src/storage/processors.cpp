...
{
    EValue val;
    val.value = std::string(value);
    val.expire_time = ttl == 0 ? 0 : std::chrono::duration_cast<std::chrono::seconds>(std::chrono::system_clock::now().time_since_epoch()).count() + ttl;
    if (storage->enable_wal_ && storage->wal_)
    {
        storage->wal_->append_log(OperationType::kSet, std::string(key), serialize(val));
    }
    storage->write_memtable(std::string(key), val);
    return true;
}
...
Response StringProcessor::execute(Storage *storage, const uint8_t type, const std::vector<std::string_view> &args)
{
    if (type == OperationType::kSet)
    {
        ...
        if (args.size() > 2)
        {
            ttl = std::stoll(std::string(args[2]));
        }
...
size_t SetProcessor::s_add(Storage *storage, const std::string_view key, const std::vector<std::string_view> &members, const bool is_recover)
{
    if (storage->enable_wal_ && storage->wal_ && !is_recover)
    {
        ...
        for (const auto &m : members)
        {
            payload += Serializer::serialize(std::string(m));
        }
        storage->wal_->append_log(OperationType::kSAdd, std::string(key), payload);
    }
    size_t added = 0;
    try
    {
        storage->memtable_->handle_value(std::string(key), [&members, &added](EValue &val) -> EValue &
                                         {
                                            if (!std::holds_alternative<std::unordered_set<std::string>>(val.value))
                                            {
                                                throw std::runtime_error("value is not a set");
                                            ...
                                            for(const auto& m : members) {
                                                if(set.insert(std::string(m)).second) added++;
                                            }
                                            return val; });
        return added;
    }
    ...
    {
        std::optional<EValue> val_opt;
        if (storage->get_from_old(std::string(key), val_opt))
        {
            EValue val = val_opt.value();
            if (!std::holds_alternative<std::unordered_set<std::string>>(val.value))
            {
            ...
            for (const auto &m : members)
            {
                if (set.insert(std::string(m)).second)
                    added++;
            }
            storage->write_memtable(std::string(key), val);
        }
...
size_t SetProcessor::s_rem(Storage *storage, const std::string_view key, const std::vector<std::string_view> &members, const bool is_recover)
{
    if (storage->enable_wal_ && storage->wal_ && !is_recover)
    {
        ...
        storage->wal_->append_log(OperationType::kSRem, std::string(key), payload);
    }
    size_t removed = 0;
    try
    {
        storage->memtable_->handle_value(std::string(key), [&members, &removed](EValue &val) -> EValue &
                                         {
                                            if(val.is_deleted()||val.is_expired()){
                                                return val;
                                            }
                                            ...
                                            else
                                            {
                                                auto &set = std::get<std::unordered_set<std::string>>(val.value);
                                                for(const auto& m : members) {
                                                    if(set.erase(std::string(m))) removed++;
                                                }
                                                return val;
                                            } });
        return removed;
...
{
    auto val_opt = storage->get(std::string(key));
...
size_t ZSetProcessor::z_add(Storage *storage, const std::string_view key, const std::vector<std::pair<std::string_view, std::string_view>> &score_members, const bool is_recover)
{
    if (storage->enable_wal_ && storage->wal_ && !is_recover)
    {
        ...
        for (const auto &p : score_members)
        {
            payload += Serializer::serialize(std::string(p.first)) + Serializer::serialize(std::string(p.second));
        }
        storage->wal_->append_log(OperationType::kZAdd, std::string(key), payload);
    }
    size_t added_count = 0;
    try
    {
        storage->memtable_->handle_value(std::string(key), [&score_members, &added_count](EValue &val) -> EValue &
                                         {
                                            if (!std::holds_alternative<ZSet>(val.value))
                                            {
                                                throw std::runtime_error("value is not a zset");
                                            ...
                                            for(const auto& p : score_members) {
                                                zset.zadd(std::string(p.second), std::string(p.first));
                                                added_count++; 
                                            }
                                            return val; });
        return added_count;
    ...
    {
        LOG_ERROR("z_add key: {}, error: {}", key, e.what());
        throw e;
    }
    return 0;
}
...
size_t ZSetProcessor::z_rem(Storage *storage, const std::string_view key, const std::vector<std::string_view> &members, const bool is_recover)
{
    if (storage->enable_wal_ && storage->wal_ && !is_recover)
    {
        ...
        storage->wal_->append_log(OperationType::kZRem, std::string(key), payload);
    }
    size_t rem_count = 0;
    try
    {
        storage->memtable_->handle_value(std::string(key), [&members, &rem_count](EValue &val) -> EValue &
                                         {
                                            if(val.is_deleted()||val.is_expired()){
                                                return val;
                                            }
    ...
    {
        LOG_ERROR("z_rem key: {}, error: {}", key, e.what());
        throw e;
    }
    return 0;
}
...
std::optional<std::string> ZSetProcessor::z_score(Storage *storage, const std::string_view key, const std::string_view member)
{
    ...
    if (!std::holds_alternative<ZSet>(val_opt.value()))
        throw std::runtime_error("value is not a zset");
    return std::get<ZSet>(val_opt.value()).zscore(std::string(member));
}
...
std::optional<size_t> ZSetProcessor::z_rank(Storage *storage, const std::string_view key, const std::string_view member)
{
    ...
    if (!std::holds_alternative<ZSet>(val_opt.value()))
        throw std::runtime_error("value is not a zset");
    return std::get<ZSet>(val_opt.value()).zrank(std::string(member));
}
...
std::string ZSetProcessor::z_incr_by(Storage *storage, const std::string_view key, const std::string_view increment, const std::string_view member, const bool is_recover)
{
    if (storage->enable_wal_ && storage->wal_ && !is_recover)
    {
        std::string payload = Serializer::serialize(std::string(increment)) + Serializer::serialize(std::string(member));
        storage->wal_->append_log(OperationType::kZIncrBy, std::string(key), payload);
    }
    std::optional<std::string> new_score;
    try
    {
        storage->memtable_->handle_value(std::string(key), [&increment, &member, &new_score](EValue &val) -> EValue &
                                         {
                                            if(val.is_deleted()||val.is_expired()){
                                                throw std::runtime_error("key not be found");
                                            }
                                            ...
                                            {
                                                auto &zset = std::get<ZSet>(val.value);
                                                new_score = zset.zincrby(std::string(member), std::string(increment));
                                                return val;
                                            } });
        return new_score == std::nullopt ? std::string("Member not be found") : new_score.value();
    }
...
std::vector<std::pair<std::string, EyaValue>> ZSetProcessor::z_range_by_score(Storage *storage, const std::string_view key, const std::string_view min, const std::string_view max)
{
    ...
    auto res = zset.zrange_by_score(std::string(min), std::string(max));
    std::vector<std::pair<std::string, EyaValue>> ret;
    for (auto &p : res)
    {
        ret.push_back({p.first, p.second});
...
size_t ZSetProcessor::z_rem_by_rank(Storage *storage, const std::string_view key, long long start, long long end, const bool is_recover)
{
    ...
    {
        std::string payload = Serializer::serialize(std::to_string(start)) + Serializer::serialize(std::to_string(end));
        storage->wal_->append_log(OperationType::kZRemByRank, std::string(key), payload);
    }
    size_t count = 0;
    try
    {
        storage->memtable_->handle_value(std::string(key), [&start, &end, &count](EValue &val) -> EValue &
                                         {
                                            if(val.is_deleted()||val.is_expired()){
                                                return val;
                                            }
...
size_t ZSetProcessor::z_rem_by_score(Storage *storage, const std::string_view key, const std::string_view min, const std::string_view max, const bool is_recover)
{
    if (storage->enable_wal_ && storage->wal_ && !is_recover)
    {
        std::string payload = Serializer::serialize(std::string(min)) + Serializer::serialize(std::string(max));
        storage->wal_->append_log(OperationType::kZRemByScore, std::string(key), payload);
    }
    size_t count = 0;
    try
    {
        storage->memtable_->handle_value(std::string(key), [&min, &max, &count](EValue &val) -> EValue &
                                         {
                                            if(val.is_deleted()||val.is_expired()){
                                                return val;
                                            }
                                            ...
                                            {
                                                auto &zset = std::get<ZSet>(val.value);
                                                count = zset.zrem_range_by_score(std::string(min), std::string(max));
                                                return val;
                                            } });
        return count;
    }
...
Response DequeProcessor::execute(Storage *storage, const uint8_t type, const std::vector<std::string_view> &args)
{
    ...
    case OperationType::kLPop:
    {
        ...
        {
            size_t count = std::stoull(std::string(args[1]));
            return Response::success(l_pop_n(storage, key, count));
        }
    }
    case OperationType::kRPop:
    ...
    case OperationType::kLRange:
        if (args.size() < 3)
            return Response::error("missing start/end");
        {
            auto vec = l_range(storage, key, std::stoll(std::string(args[1])), std::stoll(std::string(args[2])));
            return Response::success(vec);
        }
    case OperationType::kLGet:
        if (args.size() < 2)
            return Response::error("missing index");
        {
            auto v = l_get(storage, key, std::stoll(std::string(args[1])));
            return v.has_value() ? Response::success(v.value()) : Response::error("not found");
        }
    case OperationType::kLPopN:
        if (args.size() < 2)
            return Response::error("missing count");
        return Response::success(l_pop_n(storage, key, std::stoull(std::string(args[1]))));
    case OperationType::kRPopN:
        if (args.size() < 2)
            return Response::error("missing count");
        return Response::success(r_pop_n(storage, key, std::stoull(std::string(args[1]))));
    default:
        return Response::error("unsupported type");
    }
}
...
size_t DequeProcessor::l_push(Storage *storage, const std::string_view key, const std::vector<std::string_view> &values, const bool is_recover)
{
    if (storage->enable_wal_ && storage->wal_ && !is_recover)
    {
        ...
        {
            serialized_values += Serializer::serialize(std::string(v));
        }
        storage->wal_->append_log(OperationType::kLPush, std::string(key), serialized_values);
    }
    size_t size = 0;
    try
    {
        storage->memtable_->handle_value(std::string(key), [&values, &size](EValue &val) -> EValue &
                                         {
                                            if (!std::holds_alternative<std::deque<std::string>>(val.value))
                                            {
                                                throw std::runtime_error("value is not a list");
                                            ...
                                            for(const auto& value : values) {
                                                dq.push_front(std::string(value));
                                            }
                                            size = dq.size();
                                            return val; });
        return size;
...
size_t DequeProcessor::r_push(Storage *storage, const std::string_view key, const std::vector<std::string_view> &values, const bool is_recover)
{
    if (storage->enable_wal_ && storage->wal_ && !is_recover)
    {
        ...
        storage->wal_->append_log(OperationType::kRPush, std::string(key), serialized_values);
    }
    size_t size = 0;
    try
    {
...
std::optional<std::string> DequeProcessor::l_pop(Storage *storage, const std::string_view key, const bool is_recover)
{
    if (storage->enable_wal_ && storage->wal_ && !is_recover)
    {
        storage->wal_->append_log(OperationType::kLPop, std::string(key), "");
    }
    std::optional<std::string> popped_val = std::nullopt;
    try
    {
        storage->memtable_->handle_value(std::string(key), [&popped_val](EValue &val) -> EValue &
                                         {
                                            if(val.is_deleted() || val.is_expired()){
                                                return val;
                                            }
...
std::optional<std::string> DequeProcessor::r_pop(Storage *storage, const std::string_view key, const bool is_recover)
{
    if (storage->enable_wal_ && storage->wal_ && !is_recover)
    {
        storage->wal_->append_log(OperationType::kRPop, std::string(key), "");
    }
    std::optional<std::string> popped_val = std::nullopt;
    try
    {
...
std::vector<std::string> DequeProcessor::l_pop_n(Storage *storage, const std::string_view key, size_t n, const bool is_recover)
{
    if (storage->enable_wal_ && storage->wal_ && !is_recover)
    {
        storage->wal_->append_log(OperationType::kLPopN, std::string(key), Serializer::serialize(std::to_string(n)));
    }
    std::vector<std::string> popped;
    try
    {
        storage->memtable_->handle_value(std::string(key), [&](EValue &val) -> EValue &
                                         {
            if(val.is_deleted() || val.is_expired()){
                return val;
            }
...
std::vector<std::string> DequeProcessor::r_pop_n(Storage *storage, const std::string_view key, size_t n, const bool is_recover)
{
    if (storage->enable_wal_ && storage->wal_ && !is_recover)
    {
        storage->wal_->append_log(OperationType::kRPopN, std::string(key), Serializer::serialize(std::to_string(n)));
    }
    std::vector<std::string> popped;
    try
    {
...
size_t HashProcessor::h_set(Storage *storage, const std::string_view key, const std::vector<std::pair<std::string_view, std::string_view>> &field_values, const bool is_recover)
{
    if (storage->enable_wal_ && storage->wal_ && !is_recover)
    {
        ...
        {
            payload += Serializer::serialize(std::string(kv.first)) + Serializer::serialize(std::string(kv.second));
        }
        storage->wal_->append_log(OperationType::kHSet, std::string(key), payload);
    }
    size_t new_fields = 0;
    try
    {
        storage->memtable_->handle_value(std::string(key), [&field_values, &new_fields](EValue &val) -> EValue &
                                         {
                                            ...
                                            for(const auto& kv : field_values) {
                                                if (map.find(std::string(kv.first)) == map.end()) new_fields++;
                                                map[std::string(kv.first)] = std::string(kv.second);
                                            }
                                            return val; });
        return new_fields;
    }
    catch (const std::out_of_range &e)
    {
        ...
        if (storage->get_from_old(std::string(key), val_opt))
        {
            ...
            for (const auto &kv : field_values)
            {
                if (map.find(std::string(kv.first)) == map.end())
                    new_fields++;
                map[std::string(kv.first)] = std::string(kv.second);
            }
            storage->write_memtable(std::string(key), val);
    ...
    {
        LOG_ERROR("h_set key: {}, error: {}", key, e.what());
        throw e;
    }
}
...
std::optional<std::string> HashProcessor::h_get(Storage *storage, const std::string_view key, const std::string_view field)
{
    ...
    auto it = map.find(std::string(field));
    if (it != map.end())
        return it->second;
    return std::nullopt;
}
...
size_t HashProcessor::h_del(Storage *storage, const std::string_view key, const std::vector<std::string_view> &fields, const bool is_recover)
{
    if (storage->enable_wal_ && storage->wal_ && !is_recover)
    {
        ...
        {
            payload += Serializer::serialize(std::string(f));
        }
        storage->wal_->append_log(OperationType::kHDel, std::string(key), payload);
    }
    size_t deleted_count = 0;
    try
    {
        storage->memtable_->handle_value(std::string(key), [&fields, &deleted_count](EValue &val) -> EValue &
                                         {
                                            if(val.is_deleted() || val.is_expired()){
                                                return val;
                                            }
                                            ...
                                            auto &map = std::get<std::unordered_map<std::string, std::string>>(val.value);
                                            for(const auto& f : fields) {
                                                if(map.erase(std::string(f))) deleted_count++;
                                            }
                                            return val; });
    ...
    {
        LOG_ERROR("h_del key: {}, error: {}", key, e.what());
        throw e;
    }
}
...
std::unordered_map<std::string, std::string> HashProcessor::h_entries(Storage *storage, const std::string_view key)
{
    auto val_opt = storage->get(std::string(key));
    if (!val_opt.has_value())
        return {};
