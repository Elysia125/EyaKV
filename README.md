...
详细的性能测试数据和分析请查看：[**EyaKV 综合性能测试报告**](performance_test_report.md)

### 测试结果总览

基于最新的压力测试（2026-03-31），以下是关键性能指标：

| 测试场景 | 配置 | 吞吐量 | 提升倍数 |
|---------|------|--------|---------|
| **String SET (单连接)** | Pipeline (batch=100) | **69,283 ops/s** | **17.4x** vs 无 Pipeline |
| **List RPUSH (单连接)** | Pipeline (batch=100) | **21,005 ops/s** | **9.0x** vs 无 Pipeline |
| **多线程并发 (10 线程)** | Pipeline (batch=100) | **128,008 ops/s** | **6.6x** vs 无 Pipeline |
| **ZSet ZADD (单连接)** | Pipeline (batch=100) | **1,560 ops/s** | **3.6x** vs 无 Pipeline |

### 关键发现

1. **Pipeline 效果显著**：带来 **3.5-17.4 倍** 性能提升，是最有效的优化手段
2. **网络往返是瓶颈**：无 Pipeline 模式下性能损失严重
3. **数据结构性能分层**：String >> List > Set ≈ Hash > ZSet
4. **并发扩展有限**：10 线程并发效率仅 18-49%，存在优化空间
...
📊 **详细分析**：包括完整的测试数据、对比图表和优化建议，请参阅 [性能测试报告全文](performance_test_report.md)
